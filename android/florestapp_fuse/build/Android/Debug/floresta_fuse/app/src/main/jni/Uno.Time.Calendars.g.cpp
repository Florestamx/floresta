...
// internal int GetDaysInMonth(int year, int month) :181
...
{
    *__retval = __this->GetDaysInMonth(*year, *month);
}
...
// internal int GetDaysInMonth(int year, int month) [instance] :181
int GregorianYearMonthDayCalculator::GetDaysInMonth(int year, int month)
{
    int year_ = year;
    int month_ = month;
    return IsLeapYear(year_) ? uPtr(GregorianYearMonthDayCalculator::MaxDaysPerMonth())->Item<int>(month_ - 1) : uPtr(GregorianYearMonthDayCalculator::MinDaysPerMonth())->Item<int>(month_ - 1);
}
...
void YearMonthDayCalculator::ctor_(int minYear, int maxYear, int monthsInYear, int64_t averageTicksPerYear, int64_t ticksAtStartOfYear1, uArray* eras)
{
    ...
    _eras = ((uArray*)::g::Uno::Time::Preconditions::CheckNotNull(::TYPES[7/*Uno.Time.Preconditions.CheckNotNull<Uno.Time.Calendars.Era[]>*/], eras, ::STRINGS[5/*"eras"*/]));
